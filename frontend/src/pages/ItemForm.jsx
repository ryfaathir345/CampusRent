...
const ItemForm = () => {
 ...
 useEffect(() => {
  if (user && !user.isVerified) {
   toast.error('Harap unggah KTM dan tunggu verifikasi Admin untuk menambah barang');
   navigate('/profile');
  }
 }, [user, navigate]);
 ...
 <Link to="/my-items" className="inline-flex items-center gap-2 text-gray-500 hover:text-blue-600 transition-colors mb-6 font-medium">
 <ChevronLeft size={20} />
 ...
 {/* Foto Barang */}
 <div>
 <label className="form-label block mb-2">Foto Barang</label>
 <div 
 onClick={() => fileInputRef.current?.click()}
 className="w-full h-48 border-2 border-dashed border-gray-300 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-blue-500 hover:bg-blue-50/50 transition-colors relative overflow-hidden"
 >
 ...
 <img src={`${import.meta.env.VITE_API_URL?.replace('/api', '') || 'http://localhost:5000'}${p}`} alt={`Existing ${idx}`} className="h-full w-auto object-cover rounded-xl shadow-sm border border-slate-200" />
 <button
 type="button"
 ...
 <img src={p} alt={`New Preview ${idx}`} className="h-full w-auto object-cover rounded-xl shadow-sm border border-slate-200" />
 <button
 type="button"
 ...
 <div className="h-full shrink-0 aspect-square border-2 border-dashed border-gray-300 rounded-xl flex items-center justify-center hover:border-blue-500 hover:bg-blue-50/50 transition-colors text-gray-400">
 <Upload size={24} />
 </div>
 )}
 </div>
 ...
 <p className="text-sm font-medium text-gray-600">Klik untuk upload foto (Max 5)</p>
 <p className="text-xs text-gray-400 mt-1">PNG, JPG up to 2MB per file</p>
 </>
 )}
 </div>
 <input 
 ...
 className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-100 transition-colors border border-blue-200"
 >
 <MapPin size={18} />
 ...
 <Link to="/my-items" className="px-6 py-2.5 rounded-xl text-sm font-semibold text-gray-700 bg-gray-100 hover:bg-gray-200 transition-colors">
 Batal
 </Link>
